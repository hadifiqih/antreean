...
class OrderController extends Controller
{
    ...
    public function antrianDesain(){
        ...
        if(auth()->user()->role == 'sales'){
            ...
            $listSelesai = Order::with('employee', 'sales', 'job', 'user')->where('status', 2)->where('sales_id', $salesId)->orderByDesc('created_at')->take(100)->get();
            $listRevisi = [];
            ...
            $listSelesai = Order::with('employee', 'sales', 'job', 'user')->where('status', 2)->take(250)->get();
            $listRevisi = [];
        }
    ...
    public function getJobsByCategory(Request $request, string $category_id){
        // Ambil data jobs berdasarkan category_id
        $jobs = Job::where('job_name', 'like' , '%'.$request->q.'%')
                    ->select('id', 'job_name as text') // Select id dan nama pekerjaan sebagai text untuk Select2
                    ->get();
