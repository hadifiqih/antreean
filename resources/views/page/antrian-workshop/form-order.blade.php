        ...
        <tbody>
            <tr>
                ...
                <td>{{ $antrian->customer->nama }}
                    @php
                        // Menghitung jumlah antrian untuk customer tersebut
                        $totalAntrian = \App\Models\Antrian::where('customer_id', $antrian->customer_id)->count();
                    @endphp
                    @if($totalAntrian == 0)
                        <span class="badge bg-success">New Leads</span>
                    @elseif($totalAntrian == 1)
                        <span class="badge bg-warning">Pelanggan Baru</span>
                    @elseif($totalAntrian >= 2)
            ...
            @if($antrian->customer->frekuensi_order < 2)
            <tr>
